...
request options and returns a ``GuzzleHttpFork\Promise\PromiseInterface`` that is
...
``GuzzleHttpFork\HandlerStack::create(callable $handler = null)`` static method.
...
When provided no ``$handler`` argument, ``GuzzleHttpFork\HandlerStack::create()``
...
``GuzzleHttpFork\Middleware::mapRequest()`` middleware. This middleware accepts
...
``GuzzleHttpFork\Middleware::mapResponse()`` middleware.
...
.. code-block:: php
    ...
    $stack = new HandlerStack();
    $stack->setHandler(\GuzzleHttpFork\choose_handler());
...
a ``GuzzleHttpFork\Promise\PromiseInterface`` that is fulfilled with a
